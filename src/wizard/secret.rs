//! # Secret prompts
//!
//! Shared by the wizard, delegating to pimalaya-cli's OS-aware pickers:
//! [`configure_password`] offers the OS keyrings, [`configure_token`] the
//! OAuth 2.0 token brokers (Ortie, pizauth, oama), both a custom command
//! or a raw value too.
//!
//! A known provider or broker yields an argv command (a TOML array); a
//! custom command is a shell string. Sirup only reads the secret: the
//! value must already be stored, and a missing one surfaces when the
//! account is tested right after.
